'''
Person Detector and Leg Estimator based on Tensorflow Pose Estimation Project.
'''
...
def initLibrary(preview, net_width, net_height, model):
    '''
    ...
    from detectors.TFOPDetector.pose_api import PoseAPI
    person_detector_api = PoseAPI(show_preview=preview,model=model, input_width=int(net_width), input_height=int(net_height))
...
class TFOPersonDetector:
    ...
    def __init__(self, preview=True, net_width=640, net_height=368, model="cmu"):
        '''
        ...
        initLibrary(preview, net_width, net_height,model=model)
        lock.release()
    ...
    def detectPersons(self, colour_frame, gray_frame):
        ...
        global person_detector_api
        person_detector_api.show_preview = self.preview

        # Obtain results
        results = person_detector_api.processFrame(colour_frame)

        # Obtain scale factors
        results_height, results_width = person_detector_api.outputHeight, person_detector_api.outputWidth
        ...
        for i in range(len(results)):
            ...
            # Update bounds
            for j in range(len(coordinates_mapping)):
                # Do scale correction
                coordinate = _person[j]
                if coordinate is None:
                    coordinate = (0,0,0)
                else:
                    coordinate = (coordinate[1][0],coordinate[1][1],coordinate[2])
                coordinate = list(coordinate)
                coordinate[0] = int(coordinate[0] * results_width + 0.5)
                coordinate[1] = int(coordinate[1] * results_height + 0.5)
            ...
            roi = np.array(colour_frame[minY-5:maxY+5, minX-5:maxX+5], copy=True)
            person_detection.roi = roi
            ...
            person_detection.leg_point = (person_detection.central_point[0], legY)
