...
class TFODPersonDetector:
    ...
    logger = logging.getLogger("TFODPersonDetector")

    def __init__(self,
                 model_path="./detectors/TFODDetector/models/faster_rcnn_inception_v2_coco_2017_11_08/frozen_inference_graph.pb",
            ...
            for i in range(len(boxes)):
                if classes[i] == self.person_category and scores[i] > self.threshold:
                    ...
                    if i in face_detections.keys():
                        ...
                        cv2.rectangle(img, (face_detection[0], face_detection[1]),
                                      (face_detection[2], face_detection[3]), (0, 255, 0), 1)
            cv2.imshow("TFOD Preview", img)
        ...
        for i in range(len(boxes)):
            ...
            if classes[i] == self.person_category and scores[i] > self.threshold:
                ...
                person_detection.upper_body_bound = (person_detection.person_bound[0],
                                                     ...
                                                     int((person_detection.person_bound[3] +
                                                          person_detection.person_bound[1]) / 2))
                ...
                person_detection.central_point[0], person_detection.person_bound[3])
            ...
            # else:
            #     self.logger.debug("Ignoring box: %s (Score: %f)", boxes[i], scores[i])
